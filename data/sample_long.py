...
if __name__ == "__main__":
    ...
    token_num_sum = sum([args.cn, args.en, args.baike, args.arxiv, args.code, args.math, args.instruction, args.log_278, args.ai_search])
    token_num_dict = {
    ...
    for key in token_num_dict.keys():
        ...
        save_path = join(args.save_dir, f"{key}_long.jsonl")
        ...
        with open(save_path, "w", encoding = "utf-8") as f:
            for data in tqdm(sample_dataset, desc=f"Writing {key} data"):
                f.write(json.dumps({"text": data[1], "is_short": False}, ensure_ascii = False) + "\n")
